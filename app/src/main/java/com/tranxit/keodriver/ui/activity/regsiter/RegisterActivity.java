...
import com.tranxit.keodriver.driver.BuildConfig;
import com.tranxit.keodriver.driver.R;
import com.tranxit.keodriver.base.BaseActivity;
import com.tranxit.keodriver.common.SharedHelper;
import com.tranxit.keodriver.data.network.model.MyOTP;
import com.tranxit.keodriver.data.network.model.User;
import com.tranxit.keodriver.ui.activity.main.MainActivity;
import com.tranxit.keodriver.ui.activity.otp.OTPActivity;
import com.tranxit.keodriver.ui.activity.profile.ProfileActivity;
import com.tranxit.keodriver.ui.activity.welcome.WelcomeActivity;
import com.tranxit.keodriver.ui.countrypicker.Country;
import com.tranxit.keodriver.ui.countrypicker.CountryPicker;
...
import static com.tranxit.keodriver.common.CommonValidation.Validation;
import static com.tranxit.keodriver.common.CommonValidation.isValidEmail;
import static com.tranxit.keodriver.common.CommonValidation.isValidPass;
import static com.tranxit.keodriver.common.CommonValidation.isValidPhone;
import static com.tranxit.keodriver.common.Constants.APP_REQUEST_CODE;
import static com.tranxit.keodriver.common.Constants.MULTIPLE_PERMISSION;
import static com.tranxit.keodriver.common.Constants.RC_MULTIPLE_PERMISSION_CODE;
...
public class RegisterActivity extends BaseActivity implements RegisterIView {
    ...
    @BindView(R.id.txtVehicleModal)
    EditText txtVehicleModal;
    ...
    public void initView() {
      ...
      /*  if (BuildConfig.DEBUG) {
        ...
        imgProfile.setOnClickListener(view->                MultiplePermissionTask("profile"));
        imgVehicle.setOnClickListener(view -> MultiplePermissionTask("vehicle"));
...
//        }
    ...
    void MultiplePermissionTask(String s) {
        if (hasMultiplePermission()) {
            //if(s.equalsIgnoreCase("profile"))
            requestType = s;
            pickImage();

//            else if(s.equalsIgnoreCase("vehicle"))
//                pickImage2();
        } else {
    ...
    void register() {
        ...
        map.put("service_type",toRequestBody(String.valueOf(selectedServiceType)));
        map.put("service_number",toRequestBody(txtVehicleNumber.getText().toString()));
        map.put("service_model",toRequestBody(txtVehicleModal.getText().toString()));

        MultipartBody.Part filePart1 = null;
        MultipartBody.Part filePart2 = null;
        if (imgFile != null)
            filePart1 = MultipartBody.Part.createFormData("avatar", imgFile.getName(), RequestBody.create(MediaType.parse("image*//*"), imgFile));
        if (imgVehicleFile != null)
            filePart2 = MultipartBody.Part.createFormData("driver_image", imgVehicleFile.getName(), RequestBody.create(MediaType.parse("image*//*"), imgVehicleFile));
...
//                onBackPressed();
    ...
    @Override
    public void onSuccess(List<ServicesModel> response) {
        String[] list = new String[response.size()];
        for(int i =0;i<response.size();i++){
            list[i] = response.get(i).getName();
        }

        ArrayAdapter<String> ad
                = new ArrayAdapter<String>(
                this,
                R.layout.spinner_lyt,R.id.tvSpinnerItem,
                list);

        spnServiceType.setAdapter(ad);

        spnServiceType.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                selectedServiceType= response.get(i).getId();
                //Toast.makeText(RegisterActivity.this, ""+selectedServiceType, Toast.LENGTH_SHORT).show();
        ...
        EasyImage.handleActivityResult(requestCode, resultCode, data, RegisterActivity.this, new DefaultCallback() {
            ...
            public void onImagesPicked(@NonNull List<File> imageFiles, EasyImage.ImageSource source, int type) {
                ...
                if(requestType.equalsIgnoreCase("profile")){
                    ...
                    imgVehicleFile = imageFiles.get(0);
                    Glide.with(activity()).load(Uri.fromFile(imgVehicleFile)).apply(RequestOptions.placeholderOf(R.drawable.user).dontAnimate().error(R.drawable.user)).into(imgVehicle);
